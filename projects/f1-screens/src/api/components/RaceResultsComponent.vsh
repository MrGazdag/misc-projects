...
void main() {
    ...
    float fromPosition = position.x;
    float toPosition = position.y;
    if (inteq(fromPosition, -1)) {
        fromPosition = toPosition;
    } else if (inteq(toPosition, -1)) {
        toPosition = fromPosition;
    }
    float actualPosition = mix(fromPosition, toPosition, cubicInOut(timed(position.z/position.w, 0.25, 0.75)));
